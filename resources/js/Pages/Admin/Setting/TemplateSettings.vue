    ...
    <div class="tab-pane fade  " id="template" role="tabpanel">
        <form @submit.prevent="update" class="form">
            ...
            <div class="card-body  p-50">
                ...
                <!--begin::Input group-->
                <div class="row mb-6" v-for="(item,index) in template" :key='index'>
                    <!--begin::Label-->
                    ...
                    <!--end::Label-->
                    <div class="col-lg-5 fv-row" v-if="item.type === 'select'">
                         <v-select
                        ...
                        <input-error :message="templateForm.errors[item.key]" class="mt-2"/>
                    </div>
                </div>



                <div class="row mb-6" v-for="(item,index) in formpage" :key='index'>
                    ...
                    <div class="col-lg-5 fv-row" v-if="item.type === 'select'">
                         <v-select
                                class="form-control form-control-solid"
                                :options="$page.props.forms"
                                :reduce='(item) => item.id'
                                label="name"
                                v-model="templateForm[item.key]"
                                placeholder="Popop Formu Giriniz"
                            >
                                <template v-slot:no-options>{{__('no_match_record')}}</template>
                            </v-select>
                        <input-error :message="templateForm.errors[item.key]" class="mt-2"/>
                ...
                <div class="row mb-6" v-for="(item,index) in homepage" :key='index'>
                    ...
                    <div class="col-lg-5 fv-row" v-if="item.type === 'select'">
                         <v-select
                                ...
                                label="trans_title"
                                v-model="templateForm[item.key]"
                                placeholder="Anasayfa Blok Giriniz"
                            >
                                <template v-slot:no-options>{{__('no_match_record')}}</template>
                            </v-select>
                        <input-error :message="templateForm.errors[item.key]" class="mt-2"/>
            ...
            <div class="card-footer d-flex justify-content-end py-6 px-9">
                <button type="submit" class="btn btn-primary btn-sm" :disabled="templateForm.processing">
                    <span class="indicator-label" v-if="!templateForm.processing">{{__('button.save_changes')}}</span>
...
export default {
    ...
    props: ['template','homepage','formpage'],
    ...
    methods: {
        setFormData() {
            ...
            this.formpage.forEach(r => {
                formData[r.key] = parseInt(r.value);
            })
            this.templateForm = useForm('templateForm', formData)

        },
        update() {
            this.templateForm.post(route('backend.settings.update'));
        }
    }
}
</script>
